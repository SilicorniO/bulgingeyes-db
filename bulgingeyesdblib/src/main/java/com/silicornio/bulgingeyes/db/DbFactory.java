...
public class DbFactory {
	...
	public static <T>T readConfObjectFromAssets(Context context, String path, Class<T> confClass){
		try{
			...
			if(text==null){
				JEL.e("Error reading file '" + path + "' from assets");
				return null;
			}
			...
			JEL.e("There was an error reading the file '" + path + "' as a " + confClass.getName());
			e.printStackTrace();
		}
		return null;
	}
	...
	private static String readInputStream(InputStream inputStream){
		...
		try {
			...
			JEL.e("Exception reading inputstream: " + ioe.toString());
		}
